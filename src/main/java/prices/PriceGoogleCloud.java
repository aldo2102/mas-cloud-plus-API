...
//Replace this try catch block for all below subsequent examples
		try {  
			...
			if(europeCpu<asiaCpu && europeCpu<usCpu){
				VMManager.prices.setCpuPrice(europeCpu/60);
				((ModelsConfigMachines) VMManager.prices).setLocalMem("europe");
			}else if(asiaCpu<europeCpu && asiaCpu<usCpu){
				VMManager.prices.setCpuPrice(asiaCpu/60);
				((ModelsConfigMachines) VMManager.prices).setLocalCpu("asia");
			}else {
				VMManager.prices.setCpuPrice(usCpu/60);
				VMManager.prices.setLocalMem("us");
			}
			...
			if(europeMem<asiaMem && europeMem<usMem){
				VMManager.prices.setMemoryPrice(europeMem/60);
				VMManager.prices.setLocalMem("europe");
			}else if(asiaMem<europeMem && asiaMem<usMem){
				VMManager.prices.setMemoryPrice(asiaMem/60);
				VMManager.prices.setLocalMem("asia");
			}else {
				VMManager.prices.setMemoryPrice(usMem/60);
				VMManager.prices.setLocalMem("us");
			}
		...
		System.out.println("preço "+VMManager.prices.getCpuPrice());
	}
